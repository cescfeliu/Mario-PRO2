/** @file platform.cc
 *  @brief Implementación de la clase Platform
 */
...
void Platform::start_moving() {
    if (!is_moving_) {
        is_moving_ = true;
        initial_top_ = top_ - 50;
    }
}

void Platform::update() {
    if (is_moving_) {
        top_ += move_speed_ * move_direction_;
        bottom_ += move_speed_ * move_direction_;

        if (top_ >= initial_top_ + move_range_) {
            move_direction_ = -1;
        } else if (top_ <= initial_top_) {
            move_direction_ = 1;
        }
    }
}